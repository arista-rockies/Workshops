...
1) Designate a VLAN-ID for the host access ports
...
2) Designate **In-Band** mgmt addresses on a new VLAN-ID
...
- Next, let's adjust defaults to provision the network for the preexisting host IP address scheme
...
- Submit your workspace and execute the change control to push out the gateway changes to your network.

  
- Next, provision the host ports CampusB-Leaf1C Ethernet7 and CampusB-Leaf2A Ethernet7 as **switchport access** within your host VLAN
- Navigate to Provisioning, Studios, and open the **Access Interface Configuration** studio
- ![Navigate Access Interface Studio](images/openaccintstudio-h10.png)

- With the Access Pod Interfaces section, expand the **Campus: Workshop** row
...
- Set  **Enabled = Yes**
- Set **Mode = Access**
- Set VLAN to your **host VLAN ID**
- ![Leaf1C config](images/l1ceth7config-h10.png)

- Repeat the steps for CampusB-Leaf2A Ethernet7 and put the interface in the same VLAN
...
4) Setup In-band management VLAN
- Repeat the Network Hierarchy steps for adding a VLAN to add your in-band management VLAN-ID and subnet
...
- Submit the workspace and Approving and Execute the Change Control to push out the host and in-band VLAN changes to Campus B 
...
- After the replacement Leaf2A has booted back up, confirm that your customized device tags, interface configuration, and connectivity monitor probes are working on the new replacement switch.
...
- Navigate to **Provisioning**, **Actions**, **New Action** button
...
cmds = [
 "tcpdump interface any packet-count 12 filter icmp",
]
...
- Now, let's execute this action directly in this menu using the **Execute** button
